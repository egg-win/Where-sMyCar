...
struct DatabaseConditionColumnValue {
    ...
    init(condition: DBConditionType, column: String, value: Any) {
...
class LocalDatabaseManager: NSObject {
    ...
    func create(table: DBTable, columns: [DatabaseColumn], identifierMode: DBIdentifierMode = .unique) {
        if open() {
            let columnsDescription = createColumnsStatements(columns: columns, identifierMode: identifierMode)
            let createTableSQL = "CREATE TABLE \(table.rawValue) (\(columnsDescription))"
    ...
    func insertData(into table: DBTable, columnValues: [String: Any], identifierMode: DBIdentifierMode = .unique) {
        ...
        let statements = columnsStatements(from: table, columnValues: columnValues, identifierMode: identifierMode)
        
        let insertSQL = "INSERT INTO \(table.rawValue) (\(statements.columnsSQL)) VALUES (\(statements.valuesSQL))"
    ...
    func updateData(from table: DBTable, columnValues: [String: Any], conditionColumnValues: [DatabaseConditionColumnValue]) {
        guard open() else { return }
        
        let statements = updateDataStatements(columns: columnValues.map { $0.key }, conditionColumnValues: conditionColumnValues)
        
        let updateSQL = "UPDATE \(table.rawValue) SET \(statements)"
    ...
    private func updateDataStatements(columns: [String], conditionColumnValues: [DatabaseConditionColumnValue]) -> String {
        ...
        if let firstConditionColumnValue = conditionColumnValues.first {
            conditionColumns = "WHERE \(firstConditionColumnValue.column) = ?"
            for i in 0..<conditionColumnValues.count {
                if i == 0 { continue }
                conditionColumns += "\(conditionColumnValues[i].condition) \(conditionColumnValues[i].column) = ?"
            }
        }
    ...
    // MARK: - Private method
    private func createColumnDefaultValueString(from defaultValue: Any?) -> String {
    ...
    private func createColumnsStatements(columns: [DatabaseColumn], identifierMode: DBIdentifierMode) -> String {
    ...
    private func columnsStatements(from table: DBTable, columnValues: [String: Any], identifierMode: DBIdentifierMode) -> (columnsSQL: String, valuesSQL: String) {
        switch identifierMode {
        case .none:
